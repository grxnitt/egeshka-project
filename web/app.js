...
const resultPriority=school=>{const a=siteQuiz.answers;if(a.level==='low')return 'curator_score';if(Number(a.target)>=90)return school.criteria.teachers_score>=school.criteria.practice_score?'teachers_score':'practice_score';if(Number(a.budget)&&Number(a.budget)<=3000)return 'price_quality_score';return Object.keys(criteria).sort((x,y)=>school.criteria[y]-school.criteria[x])[0];};
const resultScore=school=>{const a=siteQuiz.answers,c=school.criteria;let score=school.score*10;if(a.level==='low')score+=c.curator_score*.8+c.feedback_score*.35;if(a.level==='high')score+=c.teachers_score*.45+c.practice_score*.45;if(Number(a.target)>=90)score+=c.teachers_score*.65+c.practice_score*.55;if(Number(a.budget)&&Number(a.budget)<=3000)score+=c.price_quality_score*1.15;else if(Number(a.budget)&&Number(a.budget)<=5000)score+=c.price_quality_score*.55;return score;};
